function varargout = my_spm_orthviews(action,varargin)
...
switch lower(action)
    ...
    case 'caption'
        if ~isnumeric(varargin{1})
            ...
            for i=1:numel(varargin{1})
                h = my_spm_orthviews('Caption',i,varargin{1}{i},varargin{3:end});
    ...
    case 'reload_mats'
        ...
        % redraw_all (done in my_spm_orthviews('reorient','context_quit'))
...
DeleteFcn = ['my_spm_orthviews(''Delete'',' num2str(ii) ');'];
V.ax = cell(3,1);
for i=1:3
...
if ~isempty(h)
    ...
    spm_XYZreg('Add2Reg',st.registry.hReg,st.registry.hMe, 'my_spm_orthviews');
else
...
centre = my_spm_orthviews('Pos');
my_spm_orthviews('setcoords',centre+[0 0 ScrollWheelData.VerticalScrollCount]');
%my_spm_orthviews('reposition');
...
for k = 1:numel(pluginbase)
    pluginpath = fullfile(pluginbase{k},'my_spm_orthviews');
...
for cz = numel(zl):-1:1
    ...
    item1_x = uimenu(item1, 'Label',czlabel,...
        'Callback', sprintf(...
        'my_spm_orthviews(''context_menu'',''zoom'',%d,%d)',zl(cz),rl(cz)));
...
item2     = uimenu(item_parent,'Label','Crosshairs','Callback','my_spm_orthviews(''context_menu'',''Xhair'');','Checked',checked{2});
...
item3_1   = uimenu(item3,      'Label','World space', 'Callback','my_spm_orthviews(''context_menu'',''orientation'',3);','Checked',checked{2});
item3_2   = uimenu(item3,      'Label','Voxel space (1st image)', 'Callback','my_spm_orthviews(''context_menu'',''orientation'',2);','Checked',checked{1});
item3_3   = uimenu(item3,      'Label','Voxel space (this image)', 'Callback','my_spm_orthviews(''context_menu'',''orientation'',1);','Checked','off');
...
item3_1   = uimenu(item3,      'Label','Don''t snap', 'Callback','my_spm_orthviews(''context_menu'',''snap'',3);','Checked',checked{2});
item3_2   = uimenu(item3,      'Label','Snap to 1st image', 'Callback','my_spm_orthviews(''context_menu'',''snap'',2);','Checked',checked{1});
item3_3   = uimenu(item3,      'Label','Snap to this image', 'Callback','my_spm_orthviews(''context_menu'',''snap'',1);','Checked','off');
...
item4_1   = uimenu(item4,      'Label','NN',    'Callback','my_spm_orthviews(''context_menu'',''interpolation'',3);', 'Checked',checked{3});
item4_2   = uimenu(item4,      'Label','Trilin', 'Callback','my_spm_orthviews(''context_menu'',''interpolation'',2);','Checked',checked{2});
item4_3   = uimenu(item4,      'Label','Sinc',  'Callback','my_spm_orthviews(''context_menu'',''interpolation'',1);','Checked',checked{1});

% contextsubmenu 5
% item5     = uimenu(item_parent,'Label','Position', 'Callback','my_spm_orthviews(''context_menu'',''position'');');
...
item6_1_1_1 = uimenu(item6_1_1,  'Label','Automatic', 'Callback','my_spm_orthviews(''context_menu'',''window'',2);');
item6_1_1_2 = uimenu(item6_1_1,  'Label','Manual', 'Callback','my_spm_orthviews(''context_menu'',''window'',1);');
item6_1_1_3 = uimenu(item6_1_1,  'Label','Percentiles', 'Callback','my_spm_orthviews(''context_menu'',''window'',3);');
item6_1_2   = uimenu(item6_1,    'Label','All images');
item6_1_2_1 = uimenu(item6_1_2,  'Label','Automatic', 'Callback','my_spm_orthviews(''context_menu'',''window_gl'',2);');
item6_1_2_2 = uimenu(item6_1_2,  'Label','Manual', 'Callback','my_spm_orthviews(''context_menu'',''window_gl'',1);');
if license('test','image_toolbox') == 1
    ...
    item6_2_1_1 = uimenu(item6_2_1,  'Label','Linear', 'Checked',checked{1}, ...
        'Callback','my_spm_orthviews(''context_menu'',''mapping'',''linear'');');
    item6_2_1_2 = uimenu(item6_2_1,  'Label','Equalised histogram', 'Checked',checked{2}, ...
        'Callback','my_spm_orthviews(''context_menu'',''mapping'',''histeq'');');
    item6_2_1_3 = uimenu(item6_2_1,  'Label','Equalised log-histogram', 'Checked',checked{3}, ...
        'Callback','my_spm_orthviews(''context_menu'',''mapping'',''loghisteq'');');
    item6_2_1_4 = uimenu(item6_2_1,  'Label','Equalised squared-histogram', 'Checked',checked{4}, ...
        'Callback','my_spm_orthviews(''context_menu'',''mapping'',''quadhisteq'');');
    item6_2_2   = uimenu(item6_2,    'Label','All images');
    item6_2_2_1 = uimenu(item6_2_2,  'Label','Linear', 'Checked',checked{1}, ...
        'Callback','my_spm_orthviews(''context_menu'',''mapping_gl'',''linear'');');
    item6_2_2_2 = uimenu(item6_2_2,  'Label','Equalised histogram', 'Checked',checked{2}, ...
        'Callback','my_spm_orthviews(''context_menu'',''mapping_gl'',''histeq'');');
    item6_2_2_3 = uimenu(item6_2_2,  'Label','Equalised log-histogram', 'Checked',checked{3}, ...
        'Callback','my_spm_orthviews(''context_menu'',''mapping_gl'',''loghisteq'');');
    item6_2_2_4 = uimenu(item6_2_2,  'Label','Equalised squared-histogram', 'Checked',checked{4}, ...
        'Callback','my_spm_orthviews(''context_menu'',''mapping_gl'',''quadhisteq'');');
end

% contextsubmenu 7
item7     = uimenu(item_parent,'Label','Overlay');
item7_1   = uimenu(item7,      'Label','Add blobs');
item7_1_1 = uimenu(item7_1,    'Label','This image',  'Callback','my_spm_orthviews(''context_menu'',''add_blobs'',2);');
item7_1_2 = uimenu(item7_1,    'Label','All images', 'Callback','my_spm_orthviews(''context_menu'',''add_blobs'',1);');
item7_2   = uimenu(item7,      'Label','Add image');
item7_2_1 = uimenu(item7_2,    'Label','This image',  'Callback','my_spm_orthviews(''context_menu'',''add_image'',2);');
item7_2_2 = uimenu(item7_2,    'Label','All images', 'Callback','my_spm_orthviews(''context_menu'',''add_image'',1);');
item7_3   = uimenu(item7,      'Label','Add coloured blobs','Separator','on');
item7_3_1 = uimenu(item7_3,    'Label','This image',  'Callback','my_spm_orthviews(''context_menu'',''add_c_blobs'',2);');
item7_3_2 = uimenu(item7_3,    'Label','All images', 'Callback','my_spm_orthviews(''context_menu'',''add_c_blobs'',1);');
item7_4   = uimenu(item7,      'Label','Add coloured image');
item7_4_1 = uimenu(item7_4,    'Label','This image',  'Callback','my_spm_orthviews(''context_menu'',''add_c_image'',2);');
item7_4_2 = uimenu(item7_4,    'Label','All images', 'Callback','my_spm_orthviews(''context_menu'',''add_c_image'',1);');
...
switch lower(varargin{1})
    case 'image_info'
        ...
        item9  = uimenu(varargin{2},...
            ...
            'Callback','my_spm_orthviews(''context_menu'',''swap_img'');',...
            'Separator','on');
        
    case 'repos_mm'
        oldpos_mm = my_spm_orthviews('pos');
        newpos_mm = spm_input('New Position (mm)','+1','r',sprintf('%.2f %.2f %.2f',oldpos_mm),3);
        my_spm_orthviews('reposition',newpos_mm);
        
    case 'repos_vx'
        current_handle = get_current_handle;
        oldpos_vx = my_spm_orthviews('pos', current_handle);
        newpos_vx = spm_input('New Position (voxels)','+1','r',sprintf('%.2f %.2f %.2f',oldpos_vx),3);
        newpos_mm = st.vols{current_handle}.mat*[newpos_vx;1];
        my_spm_orthviews('reposition',newpos_mm(1:3));
    ...
    case 'xhair'
        my_spm_orthviews('Xhairs',varargin{2:end});
    ...
    case 'orientation'
        ...
        elseif varargin{2} == 2,
            ...
            my_spm_orthviews('Space',get_current_handle);
    ...
    case 'window'
        ...
        elseif varargin{2} == 3
            ...
            my_spm_orthviews('window',current_handle,w);
        end
    ...
    case 'swap_img'
        ...
        if ~isempty(newimg)
            ...
            my_spm_orthviews('context_menu','image_info',get(gcbo, 'parent'));
            redraw_all;
        end
    ...
    case 'add_blobs'
        ...
        if ~isempty(SPM)
            for i = 1:numel(cm_handles)
                ...
                item7_3_1 = uimenu(c_handle,'Label','This image','Callback','my_spm_orthviews(''context_menu'',''remove_blobs'',2);');
                if varargin{2} == 1,
                    item7_3_2 = uimenu(c_handle,'Label','All images','Callback','my_spm_orthviews(''context_menu'',''remove_blobs'',1);');
                end
                ...
                set(c_handle,'Visible','on');
                delete(get(c_handle,'Children'));
                uimenu(c_handle,'Label','This image','Callback','my_spm_orthviews(''context_menu'',''setblobsmax'',2);');
                if varargin{2} == 1
                    uimenu(c_handle,'Label','All images','Callback','my_spm_orthviews(''context_menu'',''setblobsmax'',1);');
                end
            end
            redraw_all;
        end
        ...
        if ~isempty(SPM)
            ...
            for i = 1:numel(cm_handles)
                ...
                item7_4_1   = uimenu(ch_c_handle(2),'Label',hlabel,'ForegroundColor',colours(c,:),...
                    'Callback','c = get(gcbo,''UserData'');my_spm_orthviews(''context_menu'',''remove_c_blobs'',2,c);',...
                    'UserData',c);
                if varargin{2} == 1
                    item7_4_2 = uimenu(ch_c_handle(1),'Label',hlabel,'ForegroundColor',colours(c,:),...
                        'Callback','c = get(gcbo,''UserData'');my_spm_orthviews(''context_menu'',''remove_c_blobs'',1,c);',...
                        'UserData',c);
                end
            end
            redraw_all;
    ...
    case 'add_c_image'
        ...
        for k = 1:size(fname,1)
            ...
            for i = 1:numel(cm_handles)
                ...
                %set(ch_c_handle,'Visible',on');
                item7_4_1 = uimenu(ch_c_handle(2),'Label',hlabel,'ForegroundColor',colours(c,:),...
                    'Callback','c = get(gcbo,''UserData'');my_spm_orthviews(''context_menu'',''remove_c_blobs'',2,c);','UserData',c);
                if varargin{2} == 1
                    item7_4_2 = uimenu(ch_c_handle(1),'Label',hlabel,'ForegroundColor',colours(c,:),...
                        'Callback','c = get(gcbo,''UserData'');my_spm_orthviews(''context_menu'',''remove_c_blobs'',1,c);',...
                        'UserData',c);
...
for i=valid_handles(handles)
    if isfield(st.vols{i}.ax{1},'cm')
        set(findobj(st.vols{i}.ax{1}.cm,'UserData','pos_mm'),...
            'Label',sprintf('M:  %.1f %.1f %.1f',my_spm_orthviews('pos')));
        pos = my_spm_orthviews('pos',i);
